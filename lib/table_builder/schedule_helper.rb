module ScheduleHelper
  ...
  def schedule_for(objects, *args)
    raise ArgumentError, "Missing block" unless block_given?

    options = args.last.is_a?(Hash) ? args.pop : {}
    html_options = options[:html]
    builder = options[:builder] || ScheduleBuilder
  ...
  class ScheduleBuilder < TableHelper::TableBuilder
  ...
  class Calendar
    ...
    def initialize(options={})
      ...
      if options[:last] == :thirty_days || options[:last] == :thirty
        @last = @first + 30
      elsif options[:last] == :three_weeks || options[:last] == :twentyone
        @last = @first + 21
